...
const slotTimeAvgSeconds = 0.4;
const autoDeleteTimer = 60_000;

const keepMessages = process.env.KEEP_MESSAGES === 'true';
...
bot.on('callback_query', async (query) => {
  ...
  if (command === 'balances') {
    try {
      ...
      const identityBalance = await connection.getBalance(
        new PublicKey(process.env.IDENTITY_ADDRESS)
      );
      ...
      bot
        .sendMessage(
          chatId,
          `Identity Balance: ${(identityBalance / LAMPORTS_PER_SOL).toFixed(
            4
  ...
  if (command === 'rewards') {
    try {
      ...
      const rewards = await Promise.all(
        Array.from(rewardsMap)
          .filter(([key, _value]) => key < epochInfo.slotIndex + baseSlot)
          .map(async ([key, value]) => {
            if (value === 0) {
              try {
      ...
      const firstLines = ['Best Slots:']
        ...
        .concat(`\nFailed Slots:`)
        .concat(failedSlots.map((item) => `${item.slot}: ${item.rewards.toFixed(4)} SOL`));

      const message = firstLines
        .join('\n')
  ...
  if (command === 'epochInfos') {
    try {
      ...
      const validatorResponse = await fetch(
        `https://api.stakewiz.com/validator/${process.env.VOTE_ADDRESS}`
      );
      ...
      bot
        .sendMessage(
          ...
          `Epoch: ${data.epoch}\nLeader Slots: ${leaderSlots?.length || 0}\nRemaining Slots: ${
            leaderSlots.filter((num) => num > actualSlot).length
          }\nNext Slot in: ~${formatSeconds(
            parseInt((nextSlot - actualSlot) * slotTimeAvgSeconds)
          )}\nRemaining Time: ${formatSeconds(
            data.remaining_seconds
          )}\n\nActive Stake: ${new Intl.NumberFormat('en-US', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          }).format(Number(validatorData.activated_stake))} SOL\n`
        )
        .then((sentMessage) => {
          const messageId = sentMessage.message_id;
          setTimeout(() => {
  ...
  if (command === 'stakeChanges') {
    try {
      ...
      const activatingAccounts = stakedAccounts.filter(
        (stakedAccount) =>
          Number(stakedAccount.account.data.parsed.info.stake.delegation.activationEpoch) >=
            epochInfo.epoch &&
          Number(stakedAccount.account.data.parsed.info.stake.delegation.deactivationEpoch) ===
      ...
      let activating = activatingAccounts
        .map((obj) => {
          return {
            amount: obj.account.data.parsed.info.stake.delegation.stake / LAMPORTS_PER_SOL,
            formatted:
              new Intl.NumberFormat('en-US', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              }).format(obj.account.data.parsed.info.stake.delegation.stake / LAMPORTS_PER_SOL) +
              ' SOL',
          };
        })
      ...
      let deactivating = deactivatingAccounts
        .map((obj) => {
          return {
            amount: obj.account.data.parsed.info.stake.delegation.stake / LAMPORTS_PER_SOL,
            formatted:
      ...
      if (activating.length > 0) {
        const formattedActivating =
          activating.length > 50
            ? ''
        ...
        message +=
          ...
          new Intl.NumberFormat('en-US', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          }).format(totalActivating) +
          ' SOL\n\n';
      }
      if (deactivating.length > 0) {
        const formattedDeactivating =
          deactivating.length > 50
            ? ''
            : 'Deactivating:\n' + deactivating.map((entry) => `- ${entry.formatted}`).join('\n');
        message +=
          formattedDeactivating +
          '\nTotal Deactivating: ' +
          new Intl.NumberFormat('en-US', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          }).format(totalDeactivating) +
          ' SOL\n\n';
      }
      ...
      message +=
        ...
        new Intl.NumberFormat('en-US', {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }).format(totalActivating - totalDeactivating) +
        ' SOL';
...
function calculateMedianRewards(rewards) {
  const nonZeroRewards = rewards
    .filter((item) => item.rewards > 0)
    .map((item) => item.rewards)
    .sort((a, b) => a - b);

  if (nonZeroRewards.length === 0) return 0;

  const mid = Math.floor(nonZeroRewards.length / 2);
  if (nonZeroRewards.length % 2 === 0) {
    return (nonZeroRewards[mid - 1] + nonZeroRewards[mid]) / 2;
  }
  return nonZeroRewards[mid];
}
